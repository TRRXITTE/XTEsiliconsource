...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  if (status.ok()) {
    ...
    stats->AddDBStats(InternalStats::INTERNAL_NUMBER_KEYS_WRITTEN, total_count,
                      concurrent_update);
    RecordTick(stats_, NUMBER_KEYS_WRITTEN, total_count);
    stats->AddDBStats(InternalStats::INTERNAL_BYTES_WRITTEN, total_byte_size,
                      concurrent_update);
    RecordTick(stats_, BYTES_WRITTEN, total_byte_size);
    stats->AddDBStats(InternalStats::INTERNAL_WRITE_DONE_BY_SELF, 1, concurrent_update);
    RecordTick(stats_, WRITE_DONE_BY_SELF);
    auto write_done_by_other = write_group.size - 1;
    if (write_done_by_other > 0) {
      stats->AddDBStats(InternalStats::INTERNAL_WRITE_DONE_BY_OTHER, write_done_by_other,
                        concurrent_update);
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  if (w.state == WriteThread::STATE_GROUP_LEADER) {
    ...
    if (w.status.ok() && !write_options.disableWAL) {
      PERF_TIMER_GUARD(write_wal_time);
      stats->AddDBStats(InternalStats::INTERNAL_WRITE_DONE_BY_SELF, 1);
      RecordTick(stats_, WRITE_DONE_BY_SELF, 1);
      if (wal_write_group.size > 1) {
        stats->AddDBStats(InternalStats::INTERNAL_WRITE_DONE_BY_OTHER,
...
Status DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  if (status.ok()) {
    auto stats = default_cf_internal_stats_;
    if (need_log_sync) {
      stats->AddDBStats(InternalStats::INTERNAL_WAL_FILE_SYNCED, 1);
      RecordTick(stats_, WAL_FILE_SYNCED);
    }
    stats->AddDBStats(InternalStats::INTERNAL_WAL_FILE_BYTES, log_size);
    RecordTick(stats_, WAL_FILE_BYTES, log_size);
    stats->AddDBStats(InternalStats::INTERNAL_WRITE_WITH_WAL, write_with_wal);
...
Status DBImpl::ConcurrentWriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  *last_sequence = versions_->FetchAddLastAllocatedSequence(seq_inc);
  ...
  if (status.ok()) {
    ...
    auto stats = default_cf_internal_stats_;
    stats->AddDBStats(InternalStats::INTERNAL_WAL_FILE_BYTES, log_size, concurrent);
    RecordTick(stats_, WAL_FILE_BYTES, log_size);
    stats->AddDBStats(InternalStats::INTERNAL_WRITE_WITH_WAL, write_with_wal,
                      concurrent);
    RecordTick(stats_, WRITE_WITH_WAL, write_with_wal);
